    ...
    if (dir === "left" || dir === "right") {
      for (let r = 0; r < 4; r++) {
        ...
        for (let ci = 0; ci < 4; ci++) {
          if (sl[ci] === 0) continue;
          if (ti < rowTiles.length && rowTiles[ti].val === sl[ci] && !rowTiles[ti].merged) {
            const absorbed = rowTiles[ti];
            absorbed.r = r;
            absorbed.c = ci;
            absorbed.val = sl[ci];
            absorbed.merged = true;
            mergedTiles.push(absorbed);
            ti++;
          } else if (ti < rowTiles.length) {
            rowTiles[ti].r = r;
            rowTiles[ti].c = ci;
            ti++;
          }
      ...
      for (let c = 0; c < 4; c++) {
        ...
        for (let ri = 0; ri < 4; ri++) {
          if (sl[ri] === 0) continue;
          if (ti < colTiles.length && colTiles[ti].val === sl[ri] && !colTiles[ti].merged) {
            const absorbed = colTiles[ti];
            absorbed.r = ri;
            absorbed.c = c;
            absorbed.val = sl[ri];
            absorbed.merged = true;
            ti++;
          } else if (ti < colTiles.length) {
            colTiles[ti].r = ri;
            colTiles[ti].c = c;
            ti++;
          }
